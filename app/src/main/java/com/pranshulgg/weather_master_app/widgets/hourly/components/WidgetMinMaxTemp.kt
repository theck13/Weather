package com.pranshulgg.weather_master_app.widgets.hourly.components
...
fun WidgetMinMaxTemp(min: String, max: String, isSmall: Boolean = false) {
    Row() {
        Row(verticalAlignment = Alignment.CenterVertically) {
            Image(
                provider = ImageProvider(R.drawable.ic_arrow_downward_24),
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            Image(
                provider = ImageProvider(R.drawable.ic_arrow_upward_24),
                contentDescription = null,
                colorFilter = ColorFilter.tint(GlanceTheme.colors.onSurfaceVariant),
                modifier = GlanceModifier.size(if (isSmall) 18.dp else 24.dp)
            )
